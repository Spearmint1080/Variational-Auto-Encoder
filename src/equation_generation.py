...
def reconstruct_indices(sequence, number_of_letters, max_length_of_equation):
    reshaped_sequence = np.reshape(
        sequence, [max_length_of_equation, number_of_letters]
    )
...
def new_equations(
    homology, generator, latent_dimension, max_length_of_equation, tokenizer
):
    for point in homology:
        sequence = generate_equation_sequence(point, latent_dimension, generator)
        indices = reconstruct_indices(
            sequence, len(tokenizer.word_index), max_length_of_equation
        )
        print(sequence_to_smiles(indices, tokenizer))


def new_equation_generation(
    homology, generator, latent_dimension, max_length_of_equation, tokenizer
):
    list_reactions = []
    for point in homology:
...
def generate_equations(
    ...
    while len(equations) < number_of_total_equations:
        ...
        homology = calculate_equations_homology(
            equation1, equation2, number_of_samples, encoder
        )
        interpolated_equation = new_equation_generation(
            homology, generator, latent_dimension, max_length_of_equation, tokenizer
        )
...
def get_unique_equations(equations):
    ...
    return [
        ...
        if all(i not in equation for i in delete_list)
    ]
