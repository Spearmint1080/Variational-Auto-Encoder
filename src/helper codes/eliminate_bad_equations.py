...
data = pk.dump(
    eqns,
    open(
        "/nfs/home/6/tempker/aae/generated_text/Balanced_equations/all_eqs_species_transformed_as_of_01132021.pkl",
        "wb",
    ),
)
...
with open(
    "/.nfs/home/6/tempker/Dataset_creation/Dataset/Original_DATA_species_after_balancing_equations_121420.csv",
    newline="",
) as f:
    reader = csv.reader(f)
    data = list(reader)
...
with open(
    "/.nfs/home/6/tempker/aae/generated_text/VAE_generated/csv_species_list/total_new_species_152021.csv",
    "w",
) as file:
    file.writelines("%s\n" % x for x in formusho)
...
for index, lines in enumerate(splited_lines_generator(formusho, 100)):
    with open(
        "/.nfs/home/6/tempker/aae/generated_text/VAE_generated/csv_species_list/total_new_species_152021_"
        + str(index)
        + ".csv",
        "w",
    ) as f:
